...
function storyCard(s){return `<article class="story-card" data-group="${s.group}"><a class="story-thumb" href="story.html?id=${s.id}" aria-label="${s.title}"><img src="${storyImage(s)}" alt="" loading="lazy"><span class="story-number">${padId(s.id)}</span></a><div class="story-card-body"><div class="eyebrow">${s.group} · ${duration(s.start,s.end)}</div><h3><a href="story.html?id=${s.id}">${s.title}</a></h3><p>${s.teaser}</p><div class="card-meta"><strong>Primary Speaker:</strong> ${s.speaker}</div></div></article>`}
