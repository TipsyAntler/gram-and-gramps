...
renderStoryPage=function(){
  ...
  const culture=cultureMarkup(s.id),locations=locationMarkup(f),more=moreMarkup(s.id),sequence=sequenceMarkup(s);
  const people=s.people?`<div><span>People mentioned</span><strong>${speakerDisplay(s.people)}</strong></div>`:'';
  root.innerHTML=`<section class="story-hero"><img src="${storyImage(s)}" style="${storyFocusStyle(s)}" alt=""><div class="story-hero-overlay"></div><div class="story-hero-copy wrap"><div class="story-hero-meta"><div><span>Story</span><strong>${padId(s.id)}</strong></div><div><span>Primary Speaker</span><strong>${speakerDisplay(s.speaker)}</strong></div><div><span>Length</span><strong>${duration(s.start,s.end)}</strong></div></div><h1>${s.title}</h1><p>${s.teaser}</p></div></section><section class="section wrap story-layout"><div class="story-main"><div class="audio-box"><div class="audio-title"><div class="eyebrow audio-eyebrow"><span>Original Recording</span><span>December 26, 2018</span></div><h2><span class="audio-story-number">Story ${padId(s.id)}:</span><span class="audio-story-title">${s.title}</span></h2></div>${audioMarkup(s)}</div>${sequence}${quote}${culture}${context}${more}${locations}</div><aside class="fact-panel"><div><span>Era</span><strong>${s.era}</strong></div><div><span>Places</span><strong>${s.place}</strong></div>${people}</aside></section><section class="section wrap"><div class="section-head"><div><div class="eyebrow">Keep listening</div><h2>Related stories</h2></div></div><div class="story-grid compact">${related.map(storyCard).join('')}</div></section>`;
};
