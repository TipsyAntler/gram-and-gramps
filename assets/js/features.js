window.STORY_FEATURES={
  0:{
    image:'https://drive.google.com/thumbnail?id=19oRtSxif35b2OWleuBrSVDcq_XsJe4nx&sz=w1600',
    photoNote:'Family archive photo dated December 26, 2012 — six years to the day before the oral-history recording.'
  },
  1:{
    image:'https://drive.google.com/thumbnail?id=1KoYNYfRr_HRzNand59FvsZWySONmcmwL&sz=w1600',
    photoNote:'Family archive portrait from roughly the period of Gram’s early Hoboken stories; exact date and identification are still being confirmed.',
    quote:'“When I was a detective?”',
    quoteBy:'Gram',
    contextTitle:'The park has a name',
    context:'Gram’s geography points to Church Square Park in Hoboken: Our Lady of Grace overlooks the park, matching the scene she describes in the story.',
    locations:[{label:'Church Square Park',query:'Church Square Park, 400 Garden Street, Hoboken, NJ 07030',note:'A confirmed scene anchor for the Detective Story. The exact location of Dr. Peluso’s office is still being researched.'}]
  },
  2:{
    image:'https://drive.google.com/thumbnail?id=1JRgQxGl1rmDUqDWlaKU1Rv1YFn0cZvMZ&sz=w1600',
    photoNote:'A later family-archive photo chosen for the same playful family-story energy; exact event identification is still in progress.'
  },
  3:{
    image:'https://drive.google.com/thumbnail?id=19bgv_tjhUNOnBwMx3cqe-edQ-GpGiGwN&sz=w1600',
    photoNote:'Family archive childhood snapshot; exact child and date are still being confirmed.',
    quote:'“I flung open my door and knocked one guy flat on his ass.”',
    quoteBy:'Gram'
  },
  4:{
    image:'https://drive.google.com/thumbnail?id=1q9MwuVSoGHMf3uQK8c79OWD98IQhj-Kt&sz=w1600',
    photoNote:'Young-adult family archive portrait from roughly the dental-office era; identification and date are still being confirmed.'
  },
  5:{
    image:'https://drive.google.com/thumbnail?id=1LkrxufwjoBUyNscW0ik7R_eQueI-cizb&sz=w1600',
    photoNote:'Young-adult family archive portrait from roughly the dental-office era; identification and date are still being confirmed.'
  },
  6:{
    image:'https://drive.google.com/thumbnail?id=1QlgJlihSzG2NHq48RnnGE4OIcLZxD6oB&sz=w1600',
    photoNote:'Later-years family archive candid; identities and occasion are still being confirmed.'
  },
  7:{
    image:'https://drive.google.com/thumbnail?id=1ydjPtnlCK-vaKoQOYcXP6nsQCYfZFG59&sz=w1600',
    photoNote:'Family archive travel photo; exact location and date are still being confirmed.'
  },
  8:{
    image:'https://drive.google.com/thumbnail?id=1MRC4sQ7sUo91FAmOM54jIxCXMwRdP4ZP&sz=w1600',
    photoNote:'Family archive office photo — an unusually good visual fit for Gram’s working-life stories.',
    locations:[{label:'Stevens Institute of Technology',query:'Stevens Institute of Technology, Hoboken, NJ',note:'The setting for Gram’s Stevens stories. She worked there for decades, beginning full-time in 1962.'}]
  },
  9:{
    image:'https://drive.google.com/thumbnail?id=1HziTkeA3Zzl03Yn-6gV0LBizYNFS-Q_h&sz=w1600',
    photoNote:'Family archive home-life photo; identities and date are still being confirmed.'
  },
  10:{
    image:'https://drive.google.com/thumbnail?id=1uDkWC1lfMEfI4SRvEZ7ov0Cl_iW1Wmvt&sz=w1600',
    photoNote:'Middle-years family archive portrait used for Gram’s Stevens-era story; exact date is still being confirmed.',
    locations:[{label:'Stevens Institute of Technology',query:'Stevens Institute of Technology, Hoboken, NJ',note:'The setting for the Joe Introne / boss story and a major part of Gram’s working life.'}]
  },
  11:{
    image:'https://drive.google.com/thumbnail?id=1tglSKVIYo1J4gCAe8mn7T5yr_qwovAQA&sz=w1600',
    photoNote:'Family archive cake moment dated June 8, 1997 — too perfect a visual match to pass up.'
  },
  12:{
    image:'https://drive.google.com/thumbnail?id=1fwe0l1E5rrNf0e9EtvZ5JVRfEtQXPpGa&sz=w1600',
    photoNote:'Mid-century family archive event portrait; exact occasion and date are still being confirmed.'
  },
  13:{
    image:'https://drive.google.com/thumbnail?id=1c4fS4jBRjPwQsQDhCHp2Z-Pfv075R8vO&sz=w1600',
    photoNote:'Family archive file titled “1st communion 1932.jpg” — a direct childhood-era visual for Gramps’ introduction.'
  },
  14:{
    image:'https://drive.google.com/thumbnail?id=1o3TGOGPFJbwprmLPKYgwnNgg8dHlKUiO&sz=w1600',
    photoNote:'Family archive file titled “1943 Boot camp leave.jpg” — Sam in Navy uniform during leave with family.',
    contextTitle:'U.S. Navy Armed Guard',
    context:'Gramps served in the U.S. Navy from 1943–1946 as a Signalman 2nd Class in the Naval Armed Guard. Naval Armed Guard sailors were Navy personnel assigned to protect and serve aboard merchant vessels during wartime.'
  },
  15:{
    image:'https://drive.google.com/thumbnail?id=1aNm1iPt4Da3knNPaZfGOZIRrXg5eAtfp&sz=w1600',
    photoNote:'A second distinct 1940s family archive portrait from the sailor/courtship years.',
    locations:[{label:'Golden Gate Bridge',query:'Golden Gate Bridge, San Francisco, CA',note:'Gramps places the missed-signal story aboard ship while leaving San Francisco and passing the Golden Gate.'}]
  },
  16:{
    image:'https://drive.google.com/thumbnail?id=1OJq8HZqSkb7Uhd9aTO3VoW_RvdJac4Rl&sz=w1600',
    photoNote:'Early family archive church-era portrait; exact subject and date are still being confirmed.',
    locations:[{label:'Saints Peter & Paul Church',query:'Saints Peter and Paul Church, Hoboken, NJ',note:'Gramps places the teenage produce-store scam near Saints Peter & Paul. The exact vegetable store is still an open research question.'}]
  },
  17:{
    image:'https://drive.google.com/thumbnail?id=1x3E_4IKussM-jF6tnSuOe5GIQ10CoBiD&sz=w1600',
    photoNote:'A wonderfully on-theme family archive photo from roughly the same era; exact subject and date are still being confirmed.',
    locations:[
      {label:'Honolulu, Hawaiʻi',query:'Honolulu, Hawaii',note:'The setting for the hula-girl photographs Gramps mailed home.'},
      {label:'Milne Bay, Papua New Guinea',query:'Milne Bay, Papua New Guinea',note:'Gramps places the grass-skirt scam at Milne Bay during wartime service.'}
    ]
  },
  18:{
    image:'https://drive.google.com/thumbnail?id=1Uh_XdKLO6fqC6CX1g2PMrkD-f9YbdxrS&sz=w1600',
    photoNote:'Family archive file titled “Engagement Party Oct 1945.jpg” — Marietta and Sam together while he was still in Navy uniform.',
    quote:'“This is the girl I have to meet because I’m gonna marry her.”',
    quoteBy:'Gramps, remembering himself at 17',
    contextTitle:'He dates the beginning to 1942',
    context:'Gramps opens the story by placing himself just out of high school in 1942. He sees a young woman walking down the street, decides he has to meet her, and turns around to follow her.',
    locations:[
      {label:'Hoboken, New Jersey',query:'Hoboken, New Jersey',note:'The home ground for the beginning of the courtship story.'},
      {label:'Journal Square, Jersey City',query:'Journal Square, Jersey City, NJ',note:'The China Clipper restaurant Gramps remembers in the courtship story was in the Journal Square area.'}
    ]
  },
  19:{
    image:'https://drive.google.com/thumbnail?id=1T4IS4-px45gqs892SbXxOr5KlLfFpCgm&sz=w1600',
    photoNote:'Family archive high-school graduation portrait labeled “Sonny” and 1942.'
  },
  20:{
    image:'https://drive.google.com/thumbnail?id=1d68kjB_ZxVko4zeJAEXvAWkjV-rYDi9D&sz=w1600',
    photoNote:'Family archive file titled “Momma & Poppa 1930.jpg” — young Sam with his parents.',
    locations:[{label:'Hoboken Waterfront',query:'Hoboken Waterfront, Hoboken, NJ',note:'A geographic anchor for the docks and longshoreman world behind the story. The exact pier is not yet identified.'}]
  },
  21:{
    image:'https://drive.google.com/thumbnail?id=13TANxv7nL-WSxXDFlerqsGfqhA2laCxA&sz=w1600',
    photoNote:'The archive file itself is titled “Missing Hat 1944.jpg” — the most direct surviving visual for this story.',
    quote:'“So I lost my hat. And I love my hat.”',
    quoteBy:'Gramps',
    contextTitle:'A likely date: August 14, 1945',
    context:'Gramps says his ship had just reached New Westminster when Japan surrendered and he went into Vancouver for the street celebration. Vancouver archival photographs date the city’s V-J Day crowds to August 14, 1945.',
    locations:[
      {label:'Vancouver, British Columbia',query:'Vancouver, British Columbia, Canada',note:'The V-J Day street celebration where the hat disappeared.'},
      {label:'New Westminster, British Columbia',query:'New Westminster, British Columbia, Canada',note:'Gramps says his ship had just arrived here before liberty in Vancouver.'}
    ]
  },
  22:{
    image:'https://drive.google.com/thumbnail?id=1f16W1F8-D4cvUCvjxv04z0Q9WN1m_N1n&sz=w1600',
    photoNote:'The archive filename identifies this photograph as “Sam 1947.jpg,” placing it right beside the era of the music collection.'
  },
  23:{
    image:'https://drive.google.com/thumbnail?id=1T5UvQrgrGOAtP4Q4SfQmMu2uQTflucEt&sz=w1600',
    photoNote:'Family archive file titled “Busch Gardens 1975.jpg” — Sam grinning in the stocks, an unusually perfect dated visual for this run of family accidents and physical comedy.'
  },
  24:{
    image:'https://drive.google.com/thumbnail?id=1sy-qANX47sewBzPNDTN2v4QzsB9DRdDE&sz=w1600',
    photoNote:'Family archive file titled “Dad and me 1951.jpg” — a warm father-and-child image from the young-family years.'
  },
  25:{
    image:'https://drive.google.com/thumbnail?id=138JlhnXv8PEe2NGjPGMt_ol2InKzyjDQ&sz=w1600',
    photoNote:'The Drive filename identifies this photograph simply as “Mar & Sam.jpg.”'
  },
  26:{
    image:'https://drive.google.com/thumbnail?id=17DfN_xvARwh8zEA1WGOiOB2Zd_Fc0tTz&sz=w1600',
    photoNote:'Young-adult family archive portrait from roughly the dental-office era; identification and date are still being confirmed.',
    quote:'“I wasn’t going to get screwed.”',
    quoteBy:'Gram',
    contextTitle:'One story explains a whole cluster',
    context:'This is the long-form Dr. Peluso story that gives context to several earlier dental-office memories: the drinking, the wrong tooth, Gram protecting a patient, quitting, and taking the vacation pay she was owed.'
  }
};
